...
def action():
    ...
    if liste_fichiers_excel:
        ...
        numero_semaine = date_actuelle.strftime('%U').lstrip('0')
    ...
    if liste_fichiers_excel2:
        ...
        date_actuelle2 = datetime.now() + + timedelta(weeks=1)
        numero_semaine2 = date_actuelle2.strftime('%U').lstrip('0')
    ...
    for row in df_destination[f"Semaine {numero_semaine}"][5:]:
        row = ""
    print(numero_semaine)
    print(df_destination[f"Semaine {numero_semaine}"][:])
    ...
    for index, row in somme_solde_par_locataire_semaine.iterrows():
        ...
        print(numero_locataire)
        print(colonne_correspondante)
        print(numero_semaine)
    ...
    # Copiez le fichier vers un autre répertoire
    nouveau_chemin = r'C:\Users\Adam Bamba\Desktop\Projet en cours'
    shutil.copy(chemin_fichier_destination, nouveau_chemin)
